...
{
    public class RptAdjSLSurvRptGoodsDB
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLSurvRptGoodsDB(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_2581_서식_DB_종결보고서(재물).xsd";
                ...
                string sSampleDocx = myPath + @"\보고서\출력설계_2581_서식_DB_종결보고서(재물)_Head.docx";
                string sSample1Relt = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvRptGoodsDB_Head toHead = new RptAdjSLSurvRptGoodsDB_Head();
                ...
                if (dtB != null)
                {
                    for (int ii = 0; ii < dtB.Rows.Count; ii++)
                    {
                        ...
                        sSampleXSD = myPath + @"\보고서\출력설계_2581_서식_DB_종결보고서(재물)_Object.xsd";
                        ...
                        if (ObjCatgCd % 10 == 1 || ObjCatgCd % 10 == 2)  //건물,시설물
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2581_서식_DB_종결보고서(재물)_Building.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoodsDB_Building toWord = new RptAdjSLSurvRptGoodsDB_Building();
                        ...
                        else if (ObjCatgCd % 10 == 3 || ObjCatgCd % 10 == 4)  //기계기구,집기비품
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2581_서식_DB_종결보고서(재물)_Object.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoodsDB_Object toWord = new RptAdjSLSurvRptGoodsDB_Object();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                        ...
                        //    sSampleDocx = myPath + @"\보고서\출력설계_2581_서식_DB_종결보고서(재물)_Inventory.docx";
                        //    sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                        //    RptAdjSLSurvRptGoodsDB_Inventory toWord = new RptAdjSLSurvRptGoodsDB_Inventory();
                        //    sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                        //    if (sRet != "")
                        //    {
                        //        return new Response() { Result = -1, Message = sRet };
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2581_서식_DB_종결보고서(재물)_Tail.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvRptGoodsDB_Tail toTail = new RptAdjSLSurvRptGoodsDB_Tail();
                ...
                rptName = "종결보고서_재물_DB(" + sfilename + ").docx";
