...
{
    public class RptAdjSLSurvRptGoods_Head
    {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    if (dtB != null)
                    {
                        ...
                        if (!dtB.Columns.Contains("AcdtJurdPolcText")) dtB.Columns.Add("AcdtJurdPolcText"); //관할당국의견
                        {
                            var AcdtJurdPolc = dr["AcdtJurdPolc"] + "";
                                if (AcdtJurdPolc != "") AcdtJurdPolc = AcdtJurdPolc + "\n";
                            var AcdtJurdPolcOpni = dr["AcdtJurdPolcOpni"] + "";
                                if (AcdtJurdPolcOpni != "") AcdtJurdPolcOpni = AcdtJurdPolcOpni + "\n";
                            var AcdtJurdFire = dr["AcdtJurdFire"] + "";
                                if (AcdtJurdFire != "") AcdtJurdFire = AcdtJurdFire + "\n";
                            var AcdtJurdFireOpni = dr["AcdtJurdFireOpni"] + "";
                                if (AcdtJurdFireOpni != "") AcdtJurdFireOpni = AcdtJurdFireOpni + "\n";

                            var text = AcdtJurdPolc + AcdtJurdPolcOpni + AcdtJurdFire + AcdtJurdFireOpni;
                            dr["AcdtJurdPolcText"] = text;
                        }

                        if (!dtB.Columns.Contains("LegaRspsbSrcText")) dtB.Columns.Add("LegaRspsbSrcText"); //담보여부검토
                        {
                            var LegaRspsbSrc = dr["LegaRspsbSrc"] + "";
                            if (LegaRspsbSrc != "") LegaRspsbSrc = LegaRspsbSrc + "\n";
                            var LegaRspsbBss = dr["LegaRspsbBss"] + "";
                            if (LegaRspsbBss != "") LegaRspsbBss = LegaRspsbBss + "\n";
                            var CltrStpltRspsbSrc = dr["CltrStpltRspsbSrc"] + "";
                            if (CltrStpltRspsbSrc != "") CltrStpltRspsbSrc = CltrStpltRspsbSrc + "\n";
                            var CltrStpltRspsbBss = dr["CltrStpltRspsbBss"] + "";
                            if (CltrStpltRspsbBss != "") CltrStpltRspsbBss = CltrStpltRspsbBss + "\n";

                            var text = LegaRspsbSrc + LegaRspsbBss + CltrStpltRspsbSrc + CltrStpltRspsbBss;
                            dr["LegaRspsbSrcText"] = text;
                        }
                    ...
                    if (dtB != null)
                    {
                        ...
                        {
                            rUtil.ReplaceTableRow(oTbl타보험계약사항_공백.GetRow(0), "@db6OthInsurCoEmpty@", Utils.ConvertToString('.'));
                        }
                    }
