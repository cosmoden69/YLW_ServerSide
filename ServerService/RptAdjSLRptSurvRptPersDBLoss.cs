...
{
    public class RptAdjSLRptSurvRptPersDBLoss
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLRptSurvRptPersDBLoss(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_1537_서식_DB손해 종결보고서.xsd";
                ...
                string sSampleDocx = myPath + @"\보고서\출력설계_1537_서식_DB손해 종결보고서.docx";
                ...
                rptName = "종결보고서_인보험_DB손해(" + sfilename + ").docx";
        ...
        private string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile)
        {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTbl주요쟁점사항 = rUtil.GetTable(lstTable, "@B6ShrtCnts1@");
                    Table oTbl조사자확인사항 = rUtil.GetTable(lstTable, "@B7CureFrDt@");
                    ...
                    if (dtB != null)
                    {
                        ...
                        {
                            //테이블의 끝에 추가
                            rUtil.TableInsertRows(oTbl주요쟁점사항, 0, 4, dtB.Rows.Count - 1);
                        }
                    }
                    ...
                    if (dtB != null)
                    {
                        ...
                        {
                            //테이블의 끝에 추가
                            rUtil.TableInsertRow(oTbl조사자확인사항, 1, dtB.Rows.Count - 1);
                        }
                    }
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTbl표지 = rUtil.GetTable(lstTable, "@B2ChrgAdjuster@");
                    Table oTbl계약사항 = rUtil.GetTable(lstTable, "@B3InsurPrdt@");
                    Table oTbl주요쟁점사항 = rUtil.GetTable(lstTable, "@B6ShrtCnts1@");
                    Table oTbl조사자확인사항 = rUtil.GetTable(lstTable, "@B7CureFrDt@");
                    Table oTbl타사가입사항 = rUtil.GetTable(lstTable, "@B8OthINsurCoNm@");
                    ...
                    if (dtB != null && dtB.Rows.Count > 0)
                    {
                        ...
                        foreach (DataColumn col in dtB.Columns)
                        {
                            ...
                            {
                                if (Utils.ConvertToString(dr["IsrdRegno2"] + "").Replace(" ", "") != "") sValue += "-" + Utils.Left(dr["IsrdRegno2"] + "", 1) + "******";
                            }
                        ...
                        if (oTbl계약사항 != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                foreach (DataColumn col in dtB.Columns)
                                {
                                    ...
                                    if (col.ColumnName == "CtrtSttsDt") sValue = Utils.DateFormat(sValue, "yyyy.MM.dd");
                        ...
                        if (oTbl주요쟁점사항 != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                foreach (DataColumn col in dtB.Columns)
                                {
                                    sKey = rUtil.GetFieldName(sPrefix, col.ColumnName);
                                    sValue = dr[col] + "";
                                    rUtil.ReplaceTableRow(oTbl주요쟁점사항.GetRow(rnum + 0), sKey, sValue);
                                    rUtil.ReplaceTableRow(oTbl주요쟁점사항.GetRow(rnum + 1), sKey, sValue);
                                    rUtil.ReplaceTableRow(oTbl주요쟁점사항.GetRow(rnum + 2), sKey, sValue);
                                    rUtil.ReplaceTableRow(oTbl주요쟁점사항.GetRow(rnum + 3), sKey, sValue);
                                }
                            }
                        }
                    }
                        ...
                        if (oTbl조사자확인사항 != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                foreach (DataColumn col in dtB.Columns)
                                {
                                    ...
                                    rUtil.ReplaceTableRow(oTbl조사자확인사항.GetRow(i + 1), sKey, sValue);
                                }
                                if (dtB.Columns.Contains("Gubun") && dr["Gubun"] + "" == "계약일")
                                {
                                    rUtil.TableRowBackcolor(oTbl조사자확인사항.GetRow(i + 1), "ABCDEF");
                                }
                            }
                        }
                    }
                        ...
                        if (oTbl타사가입사항 != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                foreach (DataColumn col in dtB.Columns)
                                {
                                    sKey = rUtil.GetFieldName(sPrefix, col.ColumnName);
                                    sValue = dr[col] + "";
                                    if (col.ColumnName == "OthCtrtDt")
                                    {
                                        sValue = Utils.DateFormat(sValue, "yyyy.MM.dd");
                                        if (Utils.ConvertToString(dr["OthCtrtExprDt"] + "").Replace(" ", "") != "") sValue += "\r\n~ " + Utils.DateFormat(dr["OthCtrtExprDt"], "yyyy.MM.dd");
                                    }
