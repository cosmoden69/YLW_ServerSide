...
{
    public class RptAdjSLSurvRptGoods
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLSurvRptGoods(string path)
        {
            this.myPath = path;
        }
            ...
            try
            {
                ...
                if (dtB != null)
                {
                    for (int ii = 0; ii < dtB.Rows.Count; ii++)
                    {
                        ...
                        sSampleXSD = myPath + @"\보고서\출력설계_2551_서식_종결보고서(재물)_Object.xsd";
                        ...
                        if (ObjCatgCd % 10 == 1 || ObjCatgCd % 10 == 2)  //건물,시설물
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2551_서식_종결보고서(재물)_Building.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoods_Building toWord = new RptAdjSLSurvRptGoods_Building();
                        ...
                        else if (ObjCatgCd % 10 == 3 || ObjCatgCd % 10 == 4)  //기계기구,집기비품
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2551_서식_종결보고서(재물)_Object.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoods_Object toWord = new RptAdjSLSurvRptGoods_Object();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                        ...
                        else if (ObjCatgCd % 10 == 5 || ObjCatgCd % 10 == 6)  //재고자산,기타
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2551_서식_종결보고서(재물)_Inventory.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoods_Inventory toWord = new RptAdjSLSurvRptGoods_Inventory();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2551_서식_종결보고서(재물)_Tail.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvRptGoods_Tail toTail = new RptAdjSLSurvRptGoods_Tail();
