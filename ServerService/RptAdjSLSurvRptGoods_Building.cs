...
{
    public class RptAdjSLSurvRptGoods_Building
    {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    if (dtB != null)
                    {
                        ...
                        foreach (DataColumn col in dtB.Columns)
                        {
                            ...
                            if (col.ColumnName == "EvatRsltTotArea") sValue = String.Format("{0:0.00}", Utils.AddComma(Utils.ConvertToDouble(sValue))) + " ㎡"; //연면적
                            if (col.ColumnName == "EvatRsltBuyDt") sValue = Utils.Mid(sValue, 1, 4) + "." + Utils.Mid(sValue, 5, 6); //신축년월
                            if (col.ColumnName == "EvatRsltPrgMm") sValue = Math.Floor(Utils.ConvertToDouble(sValue) / 12) + "년 " + (Utils.ConvertToDouble(sValue) % 12) + "월"; //경과년수
