        ...
        private string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile)
        {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTableH = rUtil.GetTable(lstTable, "@B12VitmNm@");
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    sKey = "B4VitmNm";
                    Table oTblI = rUtil.GetTable(lstTable, sKey);
                    ...
                    if (dtB != null)
                    {
                        ...
                        for (int i = 0; i < dtB.Rows.Count; i++)
                        {
                            ...
                            foreach (DataColumn col in dtB.Columns)
                            {
                                ...
                                TableRow oRow = rUtil.GetTableRow(oTblI?.Elements<TableRow>(), sKey);
                    ...
                    if (dtB != null)
                    {
                        if (oTableF != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                {
                                    Image img = Utils.stringToImage(sValue);
                                    rUtil.SetImageNull(xrow1.GetCell(rmdr), img, 50000L, 50000L, 4000000L, 3000000L);
                                }
                                catch { }
