...
{
    public class RptAdjSLSurvMidRptGoods
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLSurvMidRptGoods(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물).xsd";
                ...
                string sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Head.docx";
                string sSample1Relt = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvMidRptGoods_Head toHead = new RptAdjSLSurvMidRptGoods_Head();
                ...
                if (drs.Length > 0)  //건물
                {
                    sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Head_Building.docx";
                    sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                    RptAdjSLSurvMidRptGoods_Head_Building toWord = new RptAdjSLSurvMidRptGoods_Head_Building();
                ...
                if (drs.Length > 0)  //기계
                {
                    sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Head_Machine.docx";
                    sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                    RptAdjSLSurvMidRptGoods_Head_Machine toWord = new RptAdjSLSurvMidRptGoods_Head_Machine();
                    sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, pds, sSampleAddFile);
                    if (sRet != "")
                    {
                        return new Response() { Result = -1, Message = sRet };
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Head_Tail.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvMidRptGoods_Head_Tail toHTail = new RptAdjSLSurvMidRptGoods_Head_Tail();
                ...
                if (dtB != null)
                {
                    for (int ii = 0; ii < dtB.Rows.Count; ii++)
                    {
                        ...
                        sSampleXSD = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Object.xsd";
                        ...
                        if (ObjCatgCd % 10 == 1 || ObjCatgCd % 10 == 2)  //건물,시설물
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Building.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvMidRptGoods_Building toWord = new RptAdjSLSurvMidRptGoods_Building();
                        ...
                        else if (ObjCatgCd % 10 == 3 || ObjCatgCd % 10 == 4)  //기계기구,집기비품
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Object.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvMidRptGoods_Object toWord = new RptAdjSLSurvMidRptGoods_Object();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                        ...
                        else if (ObjCatgCd % 10 == 5 || ObjCatgCd % 10 == 6)  //재고자산,기타
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Inventory.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvMidRptGoods_Inventory toWord = new RptAdjSLSurvMidRptGoods_Inventory();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2531_서식_중간보고서(재물)_Tail.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvMidRptGoods_Tail toTail = new RptAdjSLSurvMidRptGoods_Tail();
                ...
                rptName = "중간보고서_재물(" + sfilename + ").docx";
