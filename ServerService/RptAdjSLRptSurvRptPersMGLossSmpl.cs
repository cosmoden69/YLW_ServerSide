...
{
    public class RptAdjSLRptSurvRptPersMGLossSmpl
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLRptSurvRptPersMGLossSmpl(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_1538_서식_MG손해 종결보고서(단순).xsd";
                ...
                DataTable dtBPrg = pds.Tables["DataBlock6"];
                ...
                dtBPrg.TableName = "DataBlock6";
                pds.Tables.Remove("DataBlock6");
                pds.Tables.Add(dtBPrg);

                string sSampleDocx = myPath + @"\보고서\출력설계_1538_서식_MG손해 종결보고서(단순).docx";
        ...
        private string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile)
        {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTbl일자별확인사항 = rUtil.GetTable(lstTable, "@B6CureFrDt@");
                    Table oTbl첨부자료 = rUtil.GetTable(lstTable, "@B8FileSavSerl@");
                    Table oTbl탐문조사내역 = rUtil.GetTable(lstTable, "@B9OthInfoSeq@");
                    ...
                    if (dtB != null)
                    {
                        if (oTbl계약사항 != null)
                        {
                            ...
                            //테이블의 끝에 추가
                            rUtil.TableInsertRow(oTbl계약사항, 4, drs.Length - 1);
                        }
                    }
                    ...
                    if (dtB != null)
                    {
                        ...
                        {
                            //테이블의 끝에 추가
                            rUtil.TableInsertRow(oTbl탐문조사내역, 1, dtB.Rows.Count - 1);
                        }
                    }
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    if (dtB != null)
                    {
                        ...
                        for (int j = 0; j < drs.Length; j++)
                        {
                            DataRow dr1 = drs[j];
                            int rnum = j + 4;
                            rUtil.ReplaceTableRow(oTbl계약사항.GetRow(rnum), "@B5CltrCnts@", dr1["CltrCnts"] + "");
                            rUtil.ReplaceTableRow(oTbl계약사항.GetRow(rnum), "@B5InsurRegsAmt@", Utils.AddComma(dr1["InsurRegsAmt"]) + "원");
                        }
                        string txt1 = null;
                        int spos = 4;
                        int pos = spos;
                        ...
                        if (oTbl첨부자료 != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                foreach (DataColumn col in dtB.Columns)
                                {
                                    sKey = rUtil.GetFieldName(sPrefix, col.ColumnName);
                                    sValue = dr[col] + "";
                                    if (col.ColumnName == "FileSavDt")
                                    {
                                        sValue = Utils.DateFormat(sValue, "yyyy.MM.dd");
                                        if (sValue == "1900.01.01") sValue = "";
                                    }
                        ...
                        if (oTbl탐문조사내역 != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                foreach (DataColumn col in dtB.Columns)
                                {
                                    sKey = rUtil.GetFieldName(sPrefix, col.ColumnName);
                                    sValue = dr[col] + "";
                                    rUtil.ReplaceTableRow(oTbl탐문조사내역.GetRow(i + 1), sKey, sValue);
                                }
                            }
                        }
                    }
