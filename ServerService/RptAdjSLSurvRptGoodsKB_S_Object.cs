...
{
    public class RptAdjSLSurvRptGoodsKB_S_Object
    {
        public string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile)
        {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTbl손해액 = rUtil.GetTable(lstTable, "B4InsurObjNm");
                    ...
                    if (dtB != null)
                    {
                        ...
                        {
                            //테이블의 중간에 삽입
                            rUtil.TableInsertRow(oTbl손해액, 3, dtB.Rows.Count - 1);
                        }
                    }
                    ...
                    if (dtB != null)
                    {
                        ...
                        for (int i = 0; i < dtB.Rows.Count; i++)
                        {
                            ...
                            foreach (DataColumn col in dtB.Columns)
                            {
                                ...
                                if (col.ColumnName == "LosAmtAfDprc") sValue = Utils.AddComma(sValue);
                                rUtil.ReplaceTableRow(oTbl손해액.GetRow(i + 3), sKey, sValue);
                            }
                        }
                    }
