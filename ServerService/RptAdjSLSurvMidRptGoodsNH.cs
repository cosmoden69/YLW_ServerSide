...
{
    public class RptAdjSLSurvMidRptGoodsNH
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLSurvMidRptGoodsNH(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물).xsd";
                ...
                {
                    sSampleDocx = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물)_Head2.docx";
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물)_Head3.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                ...
                if (dtB != null)
                {
                    for (int ii = 0; ii < dtB.Rows.Count; ii++)
                    {
                        ...
                        sSampleXSD = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물)_Object.xsd";
                        ...
                        if (ObjCatgCd % 10 == 1 || ObjCatgCd % 10 == 2)  //건물,시설물
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물)_Building.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvMidRptGoodsNH_Building toWord = new RptAdjSLSurvMidRptGoodsNH_Building();
                        ...
                        else if (ObjCatgCd % 10 == 3 || ObjCatgCd % 10 == 4)  //기계기구,집기비품
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물)_Object.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvMidRptGoodsNH_Object toWord = new RptAdjSLSurvMidRptGoodsNH_Object();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                        ...
                        //    sSampleDocx = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물)_Inventory.docx";
                        //    sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                        //    RptAdjSLSurvMidRptGoodsNH_Inventory toWord = new RptAdjSLSurvMidRptGoodsNH_Inventory();
                        //    sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                        //    if (sRet != "")
                        //    {
                        //        return new Response() { Result = -1, Message = sRet };
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2565_서식_농협_진행보고서(재물)_Tail.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvMidRptGoodsNH_Tail toTail = new RptAdjSLSurvMidRptGoodsNH_Tail();
                ...
                rptName = "진행보고서_재물(" + sfilename + ").docx";
