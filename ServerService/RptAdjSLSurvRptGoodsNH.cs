...
{
    public class RptAdjSLSurvRptGoodsNH
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLSurvRptGoodsNH(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물).xsd";
                ...
                string sSampleDocx = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Head.docx";
                ...
                {
                    sSampleDocx = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Head2.docx";
                    sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                    RptAdjSLSurvRptGoodsNH_Head2 toHead2 = new RptAdjSLSurvRptGoodsNH_Head2();
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Head3.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvRptGoodsNH_Head3 toHead3 = new RptAdjSLSurvRptGoodsNH_Head3();
                ...
                if (dtB != null)
                {
                    for (int ii = 0; ii < dtB.Rows.Count; ii++)
                    {
                        ...
                        sSampleXSD = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Object.xsd";
                        ...
                        foreach (DataRow drTmp in pds.Tables["DataBlock4"].Select("ObjSeq = " + ObjSeq + " "))
                        {
                            tds.Tables["DataBlock4"].ImportRow(drTmp);
                        }
                        foreach (DataRow drTmp in pds.Tables["DataBlock5"].Select("ObjSeq = " + ObjSeq + " "))
                        {
                        ...
                        if (ObjCatgCd % 10 == 1 || ObjCatgCd % 10 == 2)  //건물,시설물
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Building.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoodsNH_Building toWord = new RptAdjSLSurvRptGoodsNH_Building();
                        ...
                        else if (ObjCatgCd % 10 == 3 || ObjCatgCd % 10 == 4)  //기계기구,집기비품
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Object.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoodsNH_Object toWord = new RptAdjSLSurvRptGoodsNH_Object();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                        ...
                        //    sSampleDocx = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Inventory.docx";
                        //    sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                        //    RptAdjSLSurvRptGoodsNH_Inventory toWord = new RptAdjSLSurvRptGoodsNH_Inventory();
                        //    sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                        //    if (sRet != "")
                        //    {
                        //        return new Response() { Result = -1, Message = sRet };
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2561_서식_농협_종결보고서(재물)_Tail.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvRptGoodsNH_Tail toTail = new RptAdjSLSurvRptGoodsNH_Tail();
                ...
                for (int ii = 0; ii < addFiles.Count; ii++)
                {
                    ...
                    RptUtils.AppendFile(mainPart, addFile, (ii > newPageCnt ? true : false));
                ...
                rptName = "종결보고서_재물(" + sfilename + ").docx";
