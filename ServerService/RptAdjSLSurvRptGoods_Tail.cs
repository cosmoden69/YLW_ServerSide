...
{
    public class RptAdjSLSurvRptGoods_Tail
    {
        public string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile)
        {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    if (dtB != null)
                    {
                        sKey = rUtil.GetFieldName(sPrefix, "TrtCdNm");
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    if (dtB != null)
                    {
                        ...
                        foreach (DataColumn col in dtB.Columns)
                        {
                            ...
                            if (col.ColumnName == "AcdtDt") if (sValue != "") { sValue = Utils.DateConv(sValue, "."); }
                            if (col.ColumnName == "AcdtTm") sValue = Utils.TimeFormat(sValue, "HH:mm");
                            if (col.ColumnName == "FldRptSbmsDt") if (sValue != "") { sValue = Utils.DateConv(sValue, "."); }
                            if (col.ColumnName == "MidRptSbmsDt") if (sValue != "") { sValue = Utils.DateConv(sValue, "."); }
                            if (col.ColumnName == "LasRptSbmsDt") if (sValue != "") { sValue = Utils.DateConv(sValue, "."); }
                        ...
                        if (oTable != null)
                        {
                            ...
                            for (int i = 0; i < dtB.Rows.Count; i++)
                            {
                                ...
                                foreach (DataColumn col in dtB.Columns)
                                {
                                    ...
                                    if (col.ColumnName == "SucBidDt") sValue = Utils.DateConv(sValue, "."); //낙찰일자
                                    if (col.ColumnName == "RmnObjAmt") sValue = Utils.AddComma(sValue); //낙찰금액
