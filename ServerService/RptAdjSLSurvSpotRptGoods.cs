...
{
    public class RptAdjSLSurvSpotRptGoods
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLSurvSpotRptGoods(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_2511_서식_현장보고서(재물).xsd";
                ...
                string sSampleDocx = myPath + @"\보고서\출력설계_2511_서식_현장보고서(재물).docx";
                ...
                {
                    sSampleDocx = myPath + @"\보고서\출력설계_2511_서식_현장보고서(재물)_Pict.docx";
                ...
                rptName = "현장보고서_재물(" + sfilename + ").docx";
        ...
        private string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile)
        {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTbl표지 = rUtil.GetTable(lstTable, "@B1CclsExptDt@");
                    Table oTbl타보험계약사항 = rUtil.GetTable(lstTable, "@B3OthInsurCo@");

                    //변수가 replace 되기 전에 테이블을 찾아 놓는다
                    sKey = rUtil.GetFieldName("B2", "InsurObjDvs");
                    Table oTableA = rUtil.GetTable(lstTable, sKey);
                    sKey = rUtil.GetFieldName("B5", "FileNo");
                    Table oTableB = rUtil.GetTable(lstTable, sKey);
                    sKey = rUtil.GetFieldName("B6", "RprtCnts");
                    Table oTableC = rUtil.GetTable(lstTable, sKey);
