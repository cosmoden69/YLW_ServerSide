...
{
    public class RptAdjSLSurvRptGoods2_Head
    {
        public string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile)
        {
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTbl피해물관련사항A = rUtil.GetTable(lstTable, "@B18DmobCarNo@");
                    Table oTbl피해물관련사항B = rUtil.GetTable(lstTable, "@B18DmobNm@");
                    ...
                    if (dtB != null)
                    {
                        if (dtB.Rows.Count < 1) dtB.Rows.Add();
                        {
                            ...
                            sValue = dr["DmobDvs"] + "";
                            //if (Convert.ToInt32(sValue) % 10 == 4) //ADJ_피해물구분:300102004(차량) 수정
                            if (Utils.ConvertToInt(sValue) % 10 == 4) //ADJ_피해물구분:300102004(차량)
                            {
                                oTbl피해물관련사항B.Remove();
                            }
                            else
