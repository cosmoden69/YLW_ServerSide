...
{
    public class RptAdjSLSurvRptLiabilityGoods_Head_Vitm
    {
        public string SetSample1(string sDocFile, string sXSDFile, DataSet pds, string sWriteFile, DataRow pdr7, DataTable pdt8)
        {
            string sRet = "";
            ...
            try
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    Table oTbl피해자 = rUtil.GetTable(lstTable, "@B7VitmNm@");
                    Table oTbl손해상황 = rUtil.GetTable(lstTable, "@B8AcdtPictImage@");
                    ...
                    if (pdt8 != null)
                    {
                        ...
                        {
                            //테이블의 끝에 추가
                            double cnt = Math.Truncate((pdt8.Rows.Count + 1) / 2.0);
                            for (int i = 1; i < cnt; i++)
                            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(sWriteFile, true))
                {
                    ...
                    sPrefix = "B7";
                    foreach (DataColumn col in pdr7.Table.Columns)
                    {
