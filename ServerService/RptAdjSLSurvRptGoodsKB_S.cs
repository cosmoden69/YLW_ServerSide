...
{
    public class RptAdjSLSurvRptGoodsKB_S
    {
        private string myPath = Application.StartupPath;

        public RptAdjSLSurvRptGoodsKB_S(string path)
        {
            this.myPath = path;
        }
        ...
        public Response GetReport(ReportParam para, ref string rptPath, ref string rptName)
        {
            try
            {
                ...
                string sSampleXSD = myPath + @"\보고서\출력설계_2591_서식_KB_종결보고서(재물, 간편).xsd";
                ...
                string sSampleDocx = myPath + @"\보고서\출력설계_2591_서식_KB_종결보고서(재물, 간편)_Head.docx";
                string sSample1Relt = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvRptGoodsKB_S_Head toHead = new RptAdjSLSurvRptGoodsKB_S_Head();
                ...
                if (dtB != null)
                {
                    for (int ii = 0; ii < dtB.Rows.Count; ii++)
                    {
                        ...
                        sSampleXSD = myPath + @"\보고서\출력설계_2591_서식_KB_종결보고서(재물, 간편)_Object.xsd";
                        ...
                        if (ObjCatgCd % 10 == 1 || ObjCatgCd % 10 == 2)  //건물,시설물
                        {
                            ...
                            sSampleDocx = myPath + @"\보고서\출력설계_2591_서식_KB_종결보고서(재물, 간편)_Building.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoodsKB_S_Building toWord = new RptAdjSLSurvRptGoodsKB_S_Building();
                        ...
                        else if (ObjCatgCd % 10 == 3 || ObjCatgCd % 10 == 4 || ObjCatgCd % 10 == 5)  //기계기구,집기비품,재고자산
                        {
                            if (ObjCatgCd % 10 == 3) drT["ObjCatgCdNm"] += "기계/기구";
                            if (ObjCatgCd % 10 == 4) drT["ObjCatgCdNm"] += "집기/비품";
                            if (ObjCatgCd % 10 == 5) drT["ObjCatgCdNm"] += "재고자산";
                            sSampleDocx = myPath + @"\보고서\출력설계_2591_서식_KB_종결보고서(재물, 간편)_Object.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoodsKB_S_Object toWord = new RptAdjSLSurvRptGoodsKB_S_Object();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                        ...
                        else if (ObjCatgCd % 10 == 6)  //기타
                        {
                            if (ObjCatgCd % 10 == 6) drT["ObjCatgCdNm"] += "기타품목";
                            sSampleDocx = myPath + @"\보고서\출력설계_2591_서식_KB_종결보고서(재물, 간편)_Inventory.docx";
                            sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                            RptAdjSLSurvRptGoodsKB_S_Inventory toWord = new RptAdjSLSurvRptGoodsKB_S_Inventory();
                            sRet = toWord.SetSample1(sSampleDocx, sSampleXSD, tds, sSampleAddFile);
                            if (sRet != "")
                            {
                                return new Response() { Result = -1, Message = sRet };
                ...
                sSampleDocx = myPath + @"\보고서\출력설계_2591_서식_KB_종결보고서(재물, 간편)_Tail.docx";
                sSampleAddFile = myPath + @"\보고서\Temp\" + Guid.NewGuid().ToString() + ".docx";
                RptAdjSLSurvRptGoodsKB_S_Tail toTail = new RptAdjSLSurvRptGoodsKB_S_Tail();
                ...
                rptName = "종결보고서_재물,간편_KB(" + sfilename + ").docx";
